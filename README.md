...
`ADependency.make`: Code overriding the dependency's initialization code taking the `ADependencyDependencyResolver` object as a parameter (e.g. `make`'s prototype could be `static func make(_ dependencies: ADependencyDependencyResolver)`).
