...
// MARK: - APIShim
final class APIShimDependencyContainer {
    private let internalDependencies: APIDependencyContainer
    let logger: Logger
    init(logger: Logger, host: String) {
        internalDependencies = APIDependencyContainer(host: host)
        self.logger = logger
    }
}
...
final class MovieManagerShimDependencyContainer {
    ...
    init() {
        internalDependencies = MovieManagerDependencyContainer()
    }
}
